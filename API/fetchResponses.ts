...
export async function fetchResponses(): Promise<ResponsesData> {
    try {
        ...
        const formattedResponses: ResponseItemData[] = responses.map((response, index) => {
            const reactionsForResponse = reactionsResults[index].data || [];
            const hasUserReacted = reactionsForResponse.some(reaction => reaction.user_id === currentUserId);

            return {
                id: response.id,
                type: response.content_type,
                content: response.content_type === 'text' ? response.text_content || '' : response.file_url || '',
 ...
 */
export async function toggleReaction(responseId: string, reactionType: string = 'like'): Promise<boolean> {
    try {
