...
export const generateAlternateUriFormats = (originalUri: string): string[] => {
  return [
    ...
    !originalUri.includes("://") && !originalUri.startsWith("/")
      ? `https://${originalUri}`
      : null,
...
export const useVideoPlayer = (initialUri: string) => {
  ...
  const initializeVideo = (uri: string) => {
    ...
    if (
      ...
      resolvedUri.includes(FileSystem.documentDirectory)
    ) {
      checkAndHandleFile(resolvedUri, uri);
    }
  };

  const checkAndHandleFile = async (
    resolvedUri: string,
    originalUri: string
  ) => {
    ...
    try {
      if (!videoState.playing) {
        try {
          ...
          console.log(
            "Direct play failed, attempting reload approach:",
            playError
          );
      ...
      const errorMessage =
        error instanceof Error ? error.message : "Unknown error playing video";
